...
class Line(object):
    ...
    def __eq__(self, other):
        return self.is_lines_equal(other)
    ...
    def is_lines_equal(self, l):
        if self.normal_vector.is_zero():
            if not l.normal_vector.is_zero():
                return False
            else:
                dif = self.constant_term - l.constant_term
                return MyDecimal(dif).is_near_zero()
    ...
    def intersection_with(self, l):
        try:
            ...
            result = Vector([x_numerator, y_numerator]).times_scalar(one_over_denom)
